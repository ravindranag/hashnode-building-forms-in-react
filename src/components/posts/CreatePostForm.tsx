...
const CreatePostForm = () => {
  ...
  const onSubmit = (data: FormData) => {
    toast.promise(mutateAsync({
      ...
      const prevData: AxiosResponse<PostDto[]> = queryClient.getQueryData(getAllPosts.getQueryKey())!
      const newData = {
        ...prevData,
        data: [
          res.data,
  ...
  return (
    <Stack component='form' gap={2} onSubmit={handleSubmit(onSubmit)}>
      ...
      <TextField
        ...
        minRows={5}
      />
      <Button variant="contained" type="submit" sx={{ alignSelf: 'end' }}>Post</Button>
    </Stack>
  )
}
